...
HISTORY_LOG_FILE = "hertz_history.json"  # New history log file 
...
class HertzJSONEncoder(json.JSONEncoder):
...
class HertzDashboard(App):
    ...
    def save_services_to_config(self):
        ...
        try:
            ...
            with open(CONFIG_FILE, "w") as f:
                json.dump(services_to_save, f, indent=4, cls=HertzJSONEncoder)
    ...
    def save_history_to_file(self, history_data):
        ...
        try:
            with open(HISTORY_LOG_FILE, "w") as f:
                json.dump(history_data, f, indent=4, cls=HertzJSONEncoder)
        ...
        try:
            ...
            if config_file.exists():
                try:
                    ...
                    display_text = f"Hertz Monitor Configuration\n\n"
                    ...
                    display_text += json.dumps(config_data, indent=4, cls=HertzJSONEncoder)
                    display_text += "\n```"
    ...
    def show_notification(self, message: str, severity: str = "information"):
        ...
        self.notify(message, title="Hertz Monitor", severity=severity, timeout=5)
        
        # Also show system notification for errors and warnings
        if severity in ["error", "warning"]:
            try:
                system_notification.notify(
                    title="Hertz Monitor Alert",
                    message=message,
                    app_name="Hertz",
                    timeout=10
                )
